...
public class InterfaceUsuario {
	...
	public static PosicaoXadrez LerPosicaoXadrez(Scanner sc) {
		try {
			...
			throw new InputMismatchException("Erro ao fornecer uma posição no xadrez, valores validos são de a1 até h8");
		}
	}
	
	public static void PrintPartida(PartidaXadrez partida, List<PecaXadrez> listaCapturadas) {
		PrintTabuleiro(partida.getPecas());
		System.out.println();
		PrintPecasCapturadas(listaCapturadas);
		System.out.println();
	...
	public static void PrintMovimentosPossiveis(PecaXadrez[][] pecas, boolean[][] movimentosPossiveis) {
		for(int i = 0; i < pecas.length; i++) {
			System.out.print((8 - i) + " ");
			for(int j = 0; j < pecas.length; j++) {
	...
	private static void PrintPeca(PecaXadrez peca, boolean colorir) {
		if(colorir) {
